...
class LinkCellContent extends Component {
    ...
    render() {
            ...
            if (typeof this.props.data === 'object'){
                ...
                return keys.map((key, index) => {
                    if (key === this.props.mainKey){
                        return <div key={index}>{(this.props.data[key] === null ||  this.props.data[key] === "") ? '-' : this.props.data[key]}<br/></div>
                    }else if(this.props.displayKeys.indexOf(key) > -1){
                        return <small key={index}>{this.props.data[key] === "" ? '-' : this.props.data[key]}<br/></small>
                    }else{
                        return ''
                    }
                })
        ...
        return <div className={this.state.textStyle}>{generateContent()}</div>
    }
}
