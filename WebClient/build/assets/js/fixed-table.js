window.fixTable = (container) => {
	...
	function relayout() {
		...
		var thStyles = ths.map(function (th, i) {
			...
			totalW += (rect.width > 250 ? 250 : rect.width);
			var style = document.defaultView.getComputedStyle(th, '');
			return {
				boundingWidth: i=== 0 && rect.width > 250 ? 250 : rect.width,
		...
		var delta = 0;
		var viewWidth = Math.max(document.documentElement.clientWidth, window.innerWidth || 0) - 18;
		if (viewWidth - totalW > 0){
			delta = Math.round((viewWidth - totalW) / (thStyles.length - 1));
		}
		
		var trHeights = tbodyTrs.map(function (tr, i) {
		...
		tbody.style.width = viewWidth < totalWidth ?  totalWidth + 'px' : (viewWidth)+ 'px';
		thead.style.width = (viewWidth < totalWidth ? totalWidth : viewWidth) - thStyles[0].boundingWidth + 'px';
		...
		ths.forEach(function (th, i) {
			...
			if (i === 0) {
				...
				th.style.width = thStyles[i].width + delta + 'px';
			}
		});
			...
			[].slice.call(tr.querySelectorAll('td'))
				.forEach(function (td, j) {
					if (j === 0) {
						...
						td.style.left = '0';
						td.style.height = trHeights[i] + 'px';
					}else{
						td.style.width = thStyles[j].width + delta + 'px';
					}
				});
		});
	}
